...
@Prefix("ssc.action.def")
class SSCBuiltinActionDefinitionSpec extends FcliBaseSpec {
    
    def "list"() {
        def args = "ssc action list -q origin=='FCLI'"
        when:
    ...
    def "help"() {
        def args = "ssc action help ${action}"
        when:
            def result = Fcli.run(args)
        then:
            verifyAll(result.stdout) {
        ...
        where:
            action << Fcli.run("ssc action list -q origin=='FCLI' -o expr={name}\\n").stdout
    }
}
