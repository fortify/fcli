...
class ListOutputOptionsSpec extends FcliBaseSpec {
    ...
    def "table-plain.no-opts"() {
        expect:
            verifyAll(generate("table-plain")) {
                success
                verifyAll(stdout) {
                    size()==23328
    ...
    def "expr"() {
        expect:
            verifyAll(generate("expr={id}: {stringValue}\n")) {
                success
                verifyAll(stdout) {
                    size()==23328
                    it[0] == '0: value1'
    ...
    def "json-properties"() {
        expect:
            verifyAll(generate("json-properties")) {
                success
                verifyAll(stdout) {
                    size()==29
                    // TODO Add expectations
