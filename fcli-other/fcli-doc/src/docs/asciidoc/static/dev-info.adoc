...
** `IInputTransformer`: Hook to transform the full JSON data before it is processed. Applied once per input in `AbstractObjectNodeProducer`.
** `IRecordTransformer`: Hook to transform individual records. Applied to each record in `AbstractObjectNodeProducer`.
** Query filtering via `QueryExpression`: Applied after record transformations to decide whether a record is included.
The previous pipeline abstraction (`JsonNodePipelineStage` & `TransformationPipelineRunner`) has been removed; transformation & filtering now happen directly inside `AbstractObjectNodeProducer` for simpler and more efficient processing.
