...
** `IInputTransformer` (deprecated): Legacy hook to transform the full JSON data before it is processed. Prefer registering a `JsonNodePipelineStage` via `StandardOutputConfig#inputStage()`.
** `IRecordTransformer` (deprecated): Legacy hook to transform individual records. Prefer registering a `JsonNodePipelineStage` via `StandardOutputConfig#recordStage()`.
** `JsonNodePipelineStage`: Unified pipeline stage abstraction (package `com.fortify.cli.common.json.producer.pipeline`) that can operate in both input and record phases.
