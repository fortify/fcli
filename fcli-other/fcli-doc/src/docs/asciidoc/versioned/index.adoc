...
* link:#_fcli_based_ci_integrations[Fcli-based CI integrations] for a unified yet customizable experience across GitHub, GitLab, Azure DevOps, and other CI/CD systems
...
== Fcli-based CI Integrations

Fcli provides seamless integration of OpenText Fortify Application Security Testing into various CI/CD platforms through the standardized `fcli action run ci` command. These integrations leverage the fcli `ci` action to provide a consistent, powerful approach to security testing across different CI systems.

Key benefits of fcli-based CI integrations:

* *Unified approach*: Same fcli `ci` action works across all CI platforms with platform-specific optimizations
* *Comprehensive functionality*: Supports both FoD and SSC backends with full SAST & SCA scanning capabilities
* *Flexible configuration*: Environment variable-based configuration with sensible defaults
* *Production-ready*: Built-in features for application version/release management, vulnerability exports, and more
* *Easy to use*: Simple setup with minimal configuration required to get started

For a complete overview of all available CI integrations and their versions, see the link:ci/index.html[CI Integrations] landing page.

Currently supported CI/CD platforms:

// This list should include all available CI integrations, pointing to the latest version of each.
// When adding new CI systems or versions, ensure this list is updated accordingly.
// See fcli-core/fcli-app/src/main/resources/com/fortify/cli/app/actions/build-time/ci-doc.yaml for CI system definitions.
* link:ci/github/v3.1.x/index.html[*Fortify GitHub Actions*]: Seamless integration for GitHub Actions workflows
* link:ci/gitlab/v2.0.x/index.html[*Fortify GitLab Components*]: Native GitLab CI/CD pipeline components
* link:ci/ado/script/index.html[*Fortify Azure DevOps*]: Script-based integration for Azure DevOps pipelines

Each integration provides platform-specific documentation including setup instructions, configuration reference, and usage examples for both FoD and SSC backends.
...
If you have `npx` available on the system where you want to install fcli and possibly other Fortify tools, *@fortify/setup* is the recommended approach for installing fcli in both CI/CD pipelines and local development environments. This lightweight npm package automatically bootstraps fcli, verifies its integrity using RSA signatures, and provides cross-platform support with minimal dependencies.

*Benefits:*

* Automatic download and verification of fcli from GitHub releases or custom URLs
* Multi-platform support (Linux, macOS, Windows)
* Flexible configuration via environment variables or config file
* Can use pre-installed fcli (3.14.0+) to skip downloads
* Consistent experience across different environments
* Full documentation: https://github.com/fortify/fortify-setup-js/blob/main/USAGE.md[fortify-setup-js USAGE.md]

*Quick Start (Linux/Mac):*

[source,bash]
----
# Initialize fcli with auto-detected version
...
Note that the `npx @fortify/setup env init` also allows for specifying additional/other Fortify tools in the `--tools` option, for example:
[source,bash]
----
# Initialize fcli with auto-detected version, and specific ScanCentral Client version
npx @fortify/setup env init --tools=fcli:auto,sc-client:25.2
----

*Configuration Examples:*

To customize fcli bootstrapping behavior, you can use the `npx @fortify/setup bootstrap-config` command as shown in the following examples. Configuration needs to be applied before running the `npx @fortify/setup env` commands.

NOTE: The following examples use command-line options to store configuration in a config file (`~/.config/fortify/setup/config.json` on Linux/Mac or `%USERPROFILE%\.config\fortify\setup\config.json` on Windows). This option-based approach is designed for manual/local installation where configuration persists across multiple commands and can be reused. For CI/CD integrations, use environment variables instead as documented in the link:ci/index.html[CI Integrations] guides to avoid multiple pipelines from interfering with each other due to sharing the same configuration file.
...
# Use pre-installed fcli (skip download)
npx @fortify/setup bootstrap-config --fcli-path=/path/to/fcli
...
*Version Pinning (Recommended for CI/CD):*

Pin @fortify/setup to at least a major version to avoid breaking changes:

[source,bash]
----
# Pin to major version (gets bug fixes and features)
npx @fortify/setup@2 env init --tools=fcli:auto
----

=== Manual Installation

If you prefer direct control over fcli installation, or if you cannot use `@fortify/setup` in your environment, you can install fcli manually or using the built-in managed installation approach. We also provide Docker images for fcli; see https://hub.docker.com/repository/docker/fortifydocker/fcli/general[fortifydocker/fcli] for information on how to run fcli using these Docker images.

==== Download and Verification

To install the fcli executables on your local system, start by visiting the fcli https://github.com/fortify/fcli/releases[Releases] page and downloading the `fcli.jar` file or appropriate native binary archive for your platform. Native binary archives will need to be extracted to a temporary directory, the `fcli.jar` file can be used as-is.
...
If you installed fcli using @fortify/setup, upgrading is straightforward.

*Linux/Mac:*

[source,bash]
----
npx @fortify/setup env init --tools=fcli:v3     # Or 'latest', or specific version
source <(npx @fortify/setup env shell)
----
...
[source,powershell]
----
# Upgrade to latest v3.x version
npx @fortify/setup env init --tools=fcli:v3     # Or 'latest', or specific version
npx @fortify/setup env powershell | Invoke-Expression
----
...
The table below describes all style elements currently supported by fcli. Fcli usage help and online manual pages also list available style elements for quick review. If you notice any style elements listed in fcli usage help that are not listed below, please link:https://github.com/fortify/fcli/issues[file a documentation bug].
...
`+no-wrap+`
|`+table+`
|By default, table cell contents will be wrapped if they don't fit cell width; use the `no-wrap` style element to abbreviate cell contents instead.

|`+fast-output+` +
`+no-fast-output+`
|`+table+`
|Fcli v3.10.0 and below used to collect all output data in memory to calculate appropriate table column widths. As of fcli v3.11.0, to allow for faster output, fcli will by default output data in increments of 100 records, sampling the contents of the first 100 records to determine column width. Use `no-fast-output` to revert to the old behavior, but note that due to other changes (taking terminal width into consideration and `(no-)wrap` style, table output may still be slightly different than before.
...
[#authenticated_http_urls]
=== Authenticated HTTP(S) URLs

Various fcli options and parameters accept either a local file path or a remote HTTP(S) URL. For remote HTTP(S) URLs, fcli also supports providing authentication details through the URL userinfo component.

Supported formats:

* Basic authentication: `+https://user:password@example.com/path+`
* Bearer token: `+https://bearer:token@example.com/path+`
* Single custom header: `+https://header:Header-Name=Header%20Value@example.com/path+`
* Multiple custom headers: `+https://headers:Header-Name=Header%20Value&Other-Header=Other%20Value@example.com/path+`

Notes:

* Any special characters in usernames, passwords, tokens, header names, or header values must be URL-encoded.
* For Basic authentication, the first `+:+` separates the username from the password; any additional `+:+` characters are treated as part of the password.
* Fcli strips the userinfo component from the request URL before sending the request and converts the userinfo contents to HTTP request headers.
* You should only use trusted HTTPS endpoints when loading remote content like actions, public keys, or tool definitions.
...
* `FCLI_DISABLE_OS_TRUSTSTORE`: Set to `true` or `1` to disable automatic merge with OS trust store certificates

When configuring trust stores through `fcli config truststore set`, use `--no-os-truststore` to persistently disable automatic
OS trust store certificate merge for that configured trust store.
...
For remote HTTP(S) URLs, these commands also support authenticated URLs as described in the link:#authenticated_http_urls[Authenticated HTTP(S) URLs] section.
