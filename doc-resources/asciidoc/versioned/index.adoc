...
* `+fcli state variable definition+` 
* `+fcli state variable contents+`

Note that the `+fcli state variable contents+` `+get+` and `+list+` commands support the regular fcli output options, and the `+list+` command also provides query capabilities. This allows for advanced use
...
fcli state variable contents list myVersions -o csv --output-to-file myVersions.csv
fcli state variable contents list myVersions -o json -q createdBy=admin --output-to-file myAdminVersions.json
fcli state variable contents list myVersions -o 'expr={id}\n' --output-to-file myVersionIds.txt
----
