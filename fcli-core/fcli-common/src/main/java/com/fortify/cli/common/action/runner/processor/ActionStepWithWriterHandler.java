    ...
    public static final List<? extends IActionStepWithHandler> createHandlers(ActionStepProcessorWith actionStepProcessorWith, ActionRunnerContextLocal ctx, ActionStepWith withStep) {
        ...
        return withWriters==null || withWriters.isEmpty() 
                ...
                : withWriters.entrySet().stream()
                    ...
                    .map(e->new ActionStepWithWriterHandler(ctx, e.getKey(), e.getValue()))
                    .toList();
    }

    @Override
    public final void doBefore() {
        ctx.getWriters().put(id, ActionStepRecordWriterFactory.createWriter(ctx, withWriter));
        ctx.getVars().set(String.format("%s.count", id), new IntNode(0));
    }
