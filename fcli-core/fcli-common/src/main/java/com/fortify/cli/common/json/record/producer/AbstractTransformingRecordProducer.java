...
/**
 * Abstract base for building record producers that apply configured input &
 * record transformations. Subclasses use Lombok-generated builders for easier
 * manual construction.
 */
@SuperBuilder
public abstract class AbstractTransformingRecordProducer implements IRecordProducer {
    @Getter
    protected final IRecordProcessingConfig outputConfig;
    private static final ObjectMapper OM = new ObjectMapper();

    private volatile boolean stopRequested; // default false
    @Override
    public final void forEach(IRecordConsumer consumer) {
        produceRawInputs(raw -> {
            if (!stopRequested) {
                processRawInput(raw, consumer, context(false, currentPageIndex++));
            }
        });
    }
    ...
    private void processRawInput(JsonNode rawInput, IRecordConsumer consumer, PipelineContext baseCtx) {
        ...
        if (stopRequested || transformed == null) {
            return;
        }
        if (transformed.isArray()) {
            ...
            while (it.hasNext() && !stopRequested) {
                if (processPotentialRecord(it.next(), consumer, context(true, baseCtx.pageIndex(), recordIndex++)) == Break.TRUE) {
                    return;
                }
            }
    ...
    private JsonNode parseObject(String s) {
        try {
            ...
            throw new FcliBugException("Error parsing record", e);
        }
    }

    private PipelineContext context(boolean recordPhase, long pageIndex) {
        return new PipelineContext(recordPhase, pageIndex, -1);
    }
    private PipelineContext context(boolean recordPhase, long pageIndex, long recordIndex) {
        return new PipelineContext(recordPhase, pageIndex, recordIndex);
    }
    private class PipelineContext implements JsonNodePipelineStage.TransformContext {
        private final boolean recordPhase;
        private final long pageIdx;
        private final long recordIdx;
        PipelineContext(boolean recordPhase, long pageIdx, long recordIdx) {
            this.recordPhase = recordPhase;
            this.pageIdx = pageIdx;
            this.recordIdx = recordIdx;
        }
        @Override
        public boolean isRecordPhase() {
            return recordPhase;
        }
        @Override
    ...
    @FunctionalInterface
    protected interface RawInputConsumer {
        void accept(JsonNode rawInput);
    }
}
