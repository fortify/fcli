...
public class OutputRecordWriterFactory {
    ...
    public IRecordWriter createRecordWriter() {
        var config = RecordWriterConfig.builder().args(resolveArgs()).style(resolveStyle()).writerSupplier(writerSupplier).build();
    ...
    private String resolveArgs() {
        return StringUtils.isNotBlank(recordWriterArgs) ? recordWriterArgs : getDefaultArgs();
    }
    ...
    private String resolveHeader(String option, String keySuffix) {
        ...
        if (header == null) {
            // For now, this method is only invoked for default options read from the
            // resource
            ...
            // if any of the above changes, we may need to apply this formatting only for
            // table
            // output for example.
