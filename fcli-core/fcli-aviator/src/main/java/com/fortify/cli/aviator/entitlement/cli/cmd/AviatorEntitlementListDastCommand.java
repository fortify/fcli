...
@Command(name = AviatorOutputHelperMixins.ListDast.CMD_NAME)
public class AviatorEntitlementListDastCommand extends AbstractAviatorAdminSessionOutputCommand {
    @Getter @Mixin private AviatorOutputHelperMixins.ListDast outputHelper;
    private static final Logger LOG = LoggerFactory.getLogger(AviatorEntitlementListDastCommand.class);
    ...
    protected JsonNode getJsonNode(AviatorAdminConfigDescriptor configDescriptor) {
        try (AviatorGrpcClient client = AviatorGrpcClientHelper.createClient(configDescriptor.getAviatorUrl())) {
            ...
            List<DastEntitlement> entitlements = client.listDastEntitlements(configDescriptor.getTenant(), signature, message);
    ...
    private ArrayNode formatEntitlements(List<DastEntitlement> entitlements) {
        ArrayNode array = AviatorGrpcUtils.createArrayNode();
        for (DastEntitlement entitlement : entitlements) {
    ...
    private void logEntitlementCount(int count, String tenant) {
        if (count == 0) {
            LOG.info("No DAST entitlements found for tenant: {}", tenant);
        } else {
            LOG.info("Successfully listed {} DAST entitlements for tenant: {}", count, tenant);
        }
    }
