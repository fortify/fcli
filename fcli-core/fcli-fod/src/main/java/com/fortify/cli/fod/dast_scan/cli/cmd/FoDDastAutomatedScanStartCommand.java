...
public class FoDDastAutomatedScanStartCommand extends AbstractFoDScanStartCommand {
    ...
    @Option(names = {"--vpn"}, descriptionKey = "fcli.fod.dast-scan.start.vpn")
    ...
    protected FoDScanDescriptor startScan(UnirestInstance unirest, FoDReleaseDescriptor releaseDescriptor) {
        ...
        try (var progressWriter = progressWriterFactory.create()) {
            ...
            if (setup == null) {
                throw new FcliSimpleException("DAST Automated scan is not configured for release '"
                        + releaseDescriptor.getReleaseName()
                        + "'. Please run one of the 'fod dast-scan setup-xxx' commands to configure the scan before starting.");
            }

            // Handle any in-progress or active scans according to configured action.
            ...
            // If the action was to not start a new scan, return current in-progress descriptor.
            ...
            try {
                ...
                // If FoD reports active scan race, resolve and return that scan instead of failing.
                if (e.getStatus() == 422 && (e.getMessage().contains("dynamic scan is currently in progress")
                        || e.getMessage().contains("errorCode: -10"))) {
