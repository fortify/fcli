...
@Command(name = "remove-trusted-url")
public class TrustStoreRemoveTrustedUrlCommand extends AbstractOutputCommand
    implements IJsonNodeSupplier, IActionCommandResultSupplier, IRecordTransformer
{
    @Getter @Mixin private OutputHelperMixins.TableNoQuery outputHelper;

    @Parameters(index = "0", paramLabel = "<url>", descriptionKey = "fcli.config.truststore.remove-trusted-url.url")
    private String url;

    @Override
    public JsonNode getJsonNode() {
        return TrustedUrlTrustStoreHelper.removeTrustedUrl(url).asJsonNode();
    }
