...
public final class MCPToolFcliRunnerFunctionStreaming implements IMCPToolRunner {
    ...
    public CallToolResult run(McpSyncServerExchange exchange, CallToolRequest request) {
        var argsNode = buildExecutionArgsNode(request);
    ...
    static ObjectNode buildExecutionArgsNode(CallToolRequest request) {
        ...
        if (request != null && request.arguments() != null) {
            for (Map.Entry<String, Object> entry : request.arguments().entrySet()) {
                if ( isPagingArgument(entry.getKey()) ) {
                    continue;
                }
    ...
    private static boolean isPagingArgument(String argName) {
        return MCPToolArgHandlerPaging.ARG_OFFSET.equals(argName)
                || MCPToolArgHandlerPaging.ARG_LIMIT.equals(argName)
                || MCPToolArgHandlerPaging.ARG_REFRESH.equals(argName);
    }
}
